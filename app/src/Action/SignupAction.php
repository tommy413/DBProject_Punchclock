...
final class SignupAction extends BaseAction
{
    ...
    public function signup(Request $request, Response $response, $args)
    {
        ...
        $orgnum = -1;
        if ($orgarr != false)$orgnum = $organarr['OrganNUM'];

        $this->logger->info($enum);

        if ($onum>=0 && $gnum>=0 && $enum>=0 && $orgnum>=0){
            $empquery = $this->sql['default']->prepare("INSERT INTO employee(EmployeeNUM , Name , OrganNUM , Passhash , Admin) VALUES ('".$enum."' , '".$account."' , '".$orgnum."' , '".$passhash."' , '')");
            $empquery->execute();
        }
        /**
